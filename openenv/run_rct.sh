#!/bin/bash
# Run OpenEnv with RollerCoaster Tycoon
# Usage: ./run_rct.sh
...
echo "🎮 OpenEnv - RollerCoaster Tycoon"
echo "================================="
...
if [ ! -d "$APPS_PATH/RCT" ]; then
    echo "Error: RCT directory not found at $APPS_PATH/RCT"
    echo "Please ensure you have downloaded RollerCoaster Tycoon to winvm/apps/RCT"
    exit 1
fi

# Run container
echo "Starting container with RollerCoaster Tycoon..."
docker run $PLATFORM_FLAG -d --name $CONTAINER_NAME \
  -p 8000:8000 \
  -p 9090:9090 \
  -e SCREEN_WIDTH=$SCREEN_WIDTH \
  -e SCREEN_HEIGHT=$SCREEN_HEIGHT \
  -e APP_FILE="/apps/RCT/RCT.exe" \
  -e APP_ARGS="" \
  -e WINDOW_TITLE="RollerCoaster Tycoon" \
